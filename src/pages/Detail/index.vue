        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spuSaleAttr,index) in skuInfo.spuSaleAttrValueList" :key="spuSaleAttr.id">
                <dt class="title">{{ spuSaleAttr.attrName }}</dt>
                <dd changepirce="0" class="{active:spuSaleAttrValue.isChecked==1}"
                    v-for="(spSaleAttrValue,index)  in spuSaleAttr.spuSaleAttrValueList"
                    :key="spuSaleAttrVlue.id">
                  {{ spuAttrValue.attrName }}
                </dd>
            ...
            <div class="cartWrap">
              ...
              <!-- 以前的路由跳转：从A路由跳转到B路由，这里在加入购物车，进行路由跳转之前，发请求把你购买的产品信息通过请求的方式通知服务器，服务器进行相应的存储
-->
              <div class="add shou">
                <a @click="addShopCar">加入购物车啊</a>
              </div>
            </div>
          </div>
        </div>
